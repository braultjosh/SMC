function write(datafilename, ts)
...
%
% REMARKS 
% It is assumed that datafilename does not exist and has to be created. An error is returned
...
datatype = com.fame.timeiq.TiqType.DOUBLE;
...
if exist(datafilename)
    sprintf('File %s already exists. Consider removing this file, changing the name of the file to be written, or use the overwrite method.', datafilename)
else
...
% Get code for ND in FAME (pass NaN values to FAME database):
